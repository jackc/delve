...
func (thread *ThreadContext) Continue() error {
	...
	return thread.handleResult(syscall.PtraceCont(thread.Id, 0))
}
...
func (thread *ThreadContext) Step() (err error) {
	...
	err = thread.handleResult(syscall.PtraceSingleStep(thread.Id))
	if err != nil {
...
func (thread *ThreadContext) handleResult(err error) error {
	...
	ps, err := wait(thread.Process.Pid)
